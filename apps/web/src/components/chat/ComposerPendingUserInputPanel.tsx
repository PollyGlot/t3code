...
const ComposerPendingUserInputCard = memo(function ComposerPendingUserInputCard({
  ...
  // The optimistic selection wins here for the same reason it wins on the option
  // buttons: the answer round-trips through the server, and the echo would
  // otherwise blink back to the previous selection for that trip.
  const selectionEcho =
    optimisticSingleSelect?.questionId === activeQuestion.id
      ? optimisticSingleSelect.optionLabel
      : derivePendingUserInputSelectionEcho(activeQuestion, progress.activeDraft);
      ...
      <div className="px-1.5 sm:px-2.5">
        ...
        >
          ...
          {/* Collapsed, the header is otherwise just a section label and a
              ...
              reminder of what is being asked, followed by the current answer so
              the card still says what was picked once the options are hidden. */}
          {isCollapsed ? (
            <span className="flex min-w-0 flex-1 items-center gap-2">
              <span className="min-w-0 truncate text-secondary-label text-xs">
                {activeQuestion.question}
              </span>
              {selectionEcho ? (
                <span className="flex min-w-0 shrink items-center gap-1 text-primary">
                  <CheckIcon className="size-3.5 shrink-0" />
                  <span className="min-w-0 truncate text-xs">{selectionEcho}</span>
                </span>
              ) : null}
            </span>
          ) : null}
